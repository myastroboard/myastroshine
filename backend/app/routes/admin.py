...
Writes are gated by ``ADMIN_ENABLED`` (on by default for single-user local
deployments). Changing ``cors_origins`` takes effect on the next restart (the
CORS middleware reads it once at startup); every other setting applies
immediately.
"""
...
async def update_app_settings(body: AppSettingsUpdate, _admin: RequireAdmin) -> AppSettingsResponse:
...
async def read_log_levels() -> LogLevels:
...
async def update_log_levels(body: LogLevelUpdate, _admin: RequireAdmin) -> LogLevels:
...
async def clear_logs(_admin: RequireAdmin) -> None:
